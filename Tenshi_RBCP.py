...
def df_to_windowed_df(dataframe, first_date_str, last_date_str, n, string):
    ...
    while True:
        ...
        values = df_subset[string].to_numpy()
...
test_date, test_x, test_y = date[val_split:-8], input[val_split:-8], output[val_split:-8]
...
mahiru_alt.compile(loss='mse', optimizer=Adam(learning_rate=0.001), metrics=['mean_absolute_error'])
mahiru_alt = keras.models.load_model('mahiru.keras')

model_file = 'ML_Application/Model/tenshi_rbcp.pkl'
joblib.dump(mahiru_alt, open(model_file, 'wb'))
